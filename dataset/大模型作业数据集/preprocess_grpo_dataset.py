...
# ==================== 配置参数 ====================
DATASET_ROOT = "/data0/jycheng/homework/MLLM_qwen3vl/dataset/大模型作业数据集/MVTEC-AD"
PREVIOUS_OUTPUT_FILE = "/data0/jycheng/homework/MLLM_qwen3vl/dataset/大模型作业数据集/output_dataset_ini.jsonl"  # 上一批
OUTPUT_FILE = "/data0/jycheng/homework/MLLM_qwen3vl/dataset/大模型作业数据集/output_dataset_grpo.jsonl"
...
def load_used_samples(prev_file: str) -> Tuple[set, int]:
    """读取上一批数据，返回已用测试图像路径集合及样本数量"""
    used = set()
    total = 0
    ...
    with open(prev_file, "r", encoding="utf-8") as f:
        for line in f:
            ...
            try:
                ...
                if "image_b_path" in sample:
                    used.add(sample["image_b_path"])
...
def process_category(category_path: str, used_test_paths: set) -> List[Dict]:
    """
    处理单个类别的数据
    ...
    for subdir in test_subdirs:
        ...
        for test_image_path in test_images:
            rel_test_path = os.path.relpath(test_image_path, start=os.path.dirname(DATASET_ROOT)).replace('\\', '/')
            if rel_test_path in used_test_paths:
                continue  # 跳过上一批已用的测试图像
            # 随机选择一张训练集正常图像作为配对
            reference_image_path = random.choice(train_good_images)
            rel_reference_path = os.path.relpath(reference_image_path, start=os.path.dirname(DATASET_ROOT)).replace('\\', '/')
            # 判断是正常样本还是异常样本
            if subdir == "good":
                # 无异常样本
                sample = {
                    "image_a_path": rel_reference_path,
                    "image_b_path": rel_test_path,
                    "label": {
                        "status": "无异常",
                        "changes": []
                    }
...
def main():
    ...
    print(f"上一批数据文件: {PREVIOUS_OUTPUT_FILE}")
    ...
    used_test_paths, target_sample_size = load_used_samples(PREVIOUS_OUTPUT_FILE)
    print(f"检测到上一批样本数: {target_sample_size}, 已用测试图像数: {len(used_test_paths)}")
    
    # 处理每个类别
    for category_path in categories:
        samples = process_category(category_path, used_test_paths)
        all_samples.extend(samples)
        print(f"当前总样本数(去重后): {len(all_samples)}\n")
    if target_sample_size:
        if len(all_samples) < target_sample_size:
            raise RuntimeError(f"可用的新测试图像不足，期望 {target_sample_size}，实际 {len(all_samples)}")
        if len(all_samples) > target_sample_size:
            all_samples = random.sample(all_samples, target_sample_size)
            print(f"样本数超过目标，按随机种子采样至 {target_sample_size}")
